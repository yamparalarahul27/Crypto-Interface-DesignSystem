...
- Icons are **Phosphor** ([phosphoricons.com](https://phosphoricons.com)), always imported by intent from `src/design-system/icons.ts` (`IconClose`, `IconPriceUp`, …): from `"../icons"` inside the system, from `"@/design-system"` in app code. **Never import `@phosphor-icons/react` directly outside `icons.ts`**; `check:portable` rule P1a fails the build, and the one seam is what keeps the family swappable and the registry dependency declared once. No new unicode glyphs or hand-drawn SVG paths for iconography: add the icon to `icons.ts` instead. See [DESIGN.md → Icons](./DESIGN.md#icons).
